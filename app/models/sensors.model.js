...
module.exports = {
    ...
    GET_SINGLE_ALL_BY_MINUTES : "SELECT s.id ,s.datetime , s.smoke_level ,s.co2_level FROM sensors_log s WHERE s.datetime >= ? AND s.datetime <= ? AND s.sensor_id = ?",

    UPDATE_SENSOR : "UPDATE sensors SET floor_id = ? , room_id = ? , updated_at = ? WHERE sensors.id = ?;",

    DELETE_SENSOR : "DELETE FROM sensors WHERE sensors.id = ?"
};
